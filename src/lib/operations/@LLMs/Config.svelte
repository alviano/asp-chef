<script context="module">
	...
	Recipe.register_operation_type(operation, async (input, options, index, id) => {
		...
		if (options.server) {
			facts.push(
				Utils.parse_atom(`${options.predicate}(server, "${LLMs.encode_string(options.server)}")`)
			);
			facts.push(
				Utils.parse_atom(
					`${options.predicate}(server_type, "${LLMs.encode_string(options.server_type)}")`
				)
			);
		}
		if (options.endpoint) {
			facts.push(
				Utils.parse_atom(
					`${options.predicate}(endpoint, "${LLMs.encode_string(options.endpoint)}")`
				)
			);
		}
		if (options.model) {
			facts.push(
				Utils.parse_atom(`${options.predicate}(model, "${LLMs.encode_string(options.model)}")`)
			);
		}
		if (options.temperature || options.temperature === 0) {
			facts.push(
				Utils.parse_atom(
					`${options.predicate}(temperature, "${LLMs.encode_string(options.temperature)}")`
				)
			);
		}
