<script>
    ...
    onMount(async () => {
        ...
        if (atom.terms.length !== 1) {
            Utils.snackbar(`Unexpected predicate ${atom.predicate}/${atom.terms.length} in #${index + 1}. Vega`);
            return;
        }
        atom = atom.terms[0];
        if (atom.string === undefined) {
            Utils.snackbar(`Unexpected non-string argument in #${index + 1}. Vega`);
            return;
        }
        ...
        try {
            ...
            Utils.snackbar(`#${index + 1}. Vega: ${err}`);
        }
    });
</script>
