<script context="module">
    ...
    const operation = "Vega";
    export const default_extra_options = {
        predicate: Option("__chart__", "Predicate containing the Vega configuration (a Base64-encoded JSON)", "predicate_name"),
...
<Operation {id} {operation} {options} {index} {default_extra_options} {add_to_recipe} {keybinding}>
    <InputGroup>
        ...
        <Input type="text" placeholder="predicate" bind:value={options.predicate} on:input={edit} data-testid="Vega-predicate" />
            ...
            {#each models as model, model_index}
                ...
                {#key model}
                    {#each model.filter(atom => atom.predicate === options.predicate) as configuration}
                        <Vega part="{model}" index="{index}" configuration_atom="{configuration}" multistage="{options.multistage}" />
                    {/each}
                {/key}
            {/each}
        </div>
