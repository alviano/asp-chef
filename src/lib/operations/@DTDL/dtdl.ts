...
export class DTDL {
    ...
    private static mapLocalizableStringProperty(
        id: string,
        name: string,
        value: string,
        res: string[]
    ...
    private static processCommand(ownerId: string, command: any, res: string[]) {
        ...
        if (command.request) {
            const reqSchemaId = this.schemaId(
        ...
        if (command.response) {
            const respSchemaId = this.schemaId(
    ...
    private static async validateDtdl(dtdlDocument: object) {
        ...
        const dtdlSchema = await fetch('/schema/dtdl.v4.schema.min.json').then((response) =>
            response.json()
        );
