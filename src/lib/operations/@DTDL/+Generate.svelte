<script>
	...
	function extractJSON(content) {
		...
		if (plainCodeBlock && plainCodeBlock[1].trim().startsWith('{')) {
		...
		// Last resort: check if the whole cleaned content is valid JSON
	...
	async function generateFromLLM() {
		if (!textualDescription.trim()) {
			Utils.snackbar('Please enter a description.');
			return;
		}

		if (!actual_config.api_key || !actual_config.api_key.startsWith('sk-')) {
			Utils.snackbar('Please configure LLM API key using @LLMs/Config first.');
			return;
		}
		...
		try {
			...
			Utils.snackbar('DTDL generated successfully!', 'success');
		} catch (error) {
			Utils.snackbar(`Generation error: ${error.message}`);
			console.error(error);
...
Extract only what is explicitly mentioned. Do not add extra features.`;
		...
		if (!jsonString) {
			console.error('Could not extract JSON from entity extraction response');
			console.error('Raw response:', response.content);
			throw new Error('LLM did not return valid JSON for entity extraction. Try using a chat model like "meta-llama/llama-3.1-8b-instruct" instead of a code completion model.');
		}

		console.log('Extracted entity JSON string:', jsonString);

		let extracted;
		try {
			// Try relaxed JSON parser first (handles comments, trailing commas, etc)
			...
			try {
				...
				console.error('Entity JSON parse error:', parseError);
				console.error('Entity JSON string that failed:', jsonString);
				throw new Error(`Failed to parse entity extraction JSON: ${parseError.message}`);
			}
		}
	...
	async function validateWithASP(extracted) {
		// Genera programma ASP per validazione constraints
...
${constraints}`;
		...
		if (!jsonString) {
			...
			console.error('Raw response:', response.content);
			throw new Error('LLM did not return valid JSON. Check the "Raw LLM Output" section below. Try using a chat model like "meta-llama/llama-3.1-8b-instruct" instead of a code completion model.');
		}
		...
		try {
			...
			for (const iface of interfaces) {
				...
				if (iface['@type'] !== 'Interface') {
					throw new Error(`Expected @type to be "Interface", got "${iface['@type']}"`);
				}
			}
			...
			try {
				...
				validationResult = {
					...
					warning: 'DTDL structure is valid but schema validation failed. The model might still be usable.'
				};
			}
		} catch (error) {
			validationResult = { valid: false, error: error.message };
...
`;
	...
	async function sendToPipeline() {
		...
		try {
			...
			Utils.snackbar(`Error sending to pipeline: ${error.message}`);
		}
	}

	async function generateFromTemplate() {
		if (!templateParams.name || templateParams.name.trim() === '') {
			Utils.snackbar('Please enter an interface name.');
			return;
		}
		...
		try {
			...
			const dtdl = {
				...
				description: templateParams.description || `${templates[selectedTemplate].name} interface`,
				contents: contents
			};
			...
			Utils.snackbar('DTDL generated from template!', 'success');
		} catch (error) {
			Utils.snackbar(`Generation error: ${error.message}`);
			console.error(error);
			validationResult = { valid: false, error: error.message };
	...
	async function generateFromSchema() {
		if (!jsonSchemaInput || jsonSchemaInput.trim() === '') {
			Utils.snackbar('Please enter a JSON Schema.');
			return;
		}
		...
		try {
			...
			if (!schema.title && !schema.$id) {
				throw new Error('JSON Schema must have a "title" or "$id" field');
			}
			...
			Utils.snackbar('DTDL generated from JSON Schema!', 'success');
		} catch (error) {
			Utils.snackbar(`Conversion error: ${error.message}`);
			console.error(error);
			validationResult = { valid: false, error: error.message };
		} finally {
			isProcessing = false;
	...
	function mapJsonSchemaToDTDL(type, propDef) {
		switch (type) {
			...
			case 'array':
				return {
					'@type': 'Array',
					elementSchema: mapJsonSchemaToDTDL(propDef.items?.type || 'string', propDef.items || {})
				};
			case 'object':
		...
		<div class="pt-3">
				...
				<Card class="mb-3 border-warning">
					<CardBody>
						...
						<details>
							...
							<pre class="code-preview mt-2" style="max-height: 300px; font-size: 0.75rem;">{rawLLMOutput}</pre>
						</details>
					</CardBody>
				</Card>
			{/if}
			...
			{#if aspConstraints}
				<Alert color="info" fade={false}>
					...
					<pre class="mb-0 mt-2" style="font-size: 0.875rem;">{aspConstraints}</pre>
				</Alert>
			{/if}
					...
					<CardBody>
						...
						{#each templateParams.telemetries as tel, i}
							<div class="row mb-2">
								...
								<div class="col-md-3">
									<Input type="select" bind:value={tel.schema} bsSize="sm" disabled={isProcessing}>
					...
					<CardBody>
						...
						{#each templateParams.properties as prop, i}
							<div class="row mb-2">
								...
								<div class="col-md-3">
									<Input type="select" bind:value={prop.schema} bsSize="sm" disabled={isProcessing}>
										<option value="string">string</option>
										<option value="double">double</option>
										<option value="integer">integer</option>
										<option value="boolean">boolean</option>
