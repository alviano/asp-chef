...
Now translate this query:`;
		...
		generatedASP = typeof response.content === 'string' ? response.content : response.content.map(c => typeof c === 'string' ? c : c.text).join('');
	...
	async function generateNaturalLanguageAnswer() {
		...
		naturalLanguageAnswer = typeof response.content === 'string' ? response.content : response.content.map((block) => block.text || '').join('');
	}
		...
		<div>
			...
			<div class="mb-3">
				...
				<div class="d-flex flex-wrap gap-2">
					{#each exampleQueries as example}
						<Button size="sm" color="secondary" outline on:click={() => useExampleQuery(example)}>
							{example}
						</Button>
					{/each}
				</div>
			...
			{#if queryResults.length > 0}
				<div class="mb-3">
					...
					<div class="result-box bg-light p-3 rounded" style="max-height: 400px; overflow-y: auto;">
