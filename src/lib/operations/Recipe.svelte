        ...
        {#key ingredients}
            {#each ingredients as item}
                ...
                {:else if Recipe.has_operation_type(`@deprecated/${item.operation}`)}
                    <svelte:component this={Recipe.operation_component(`@deprecated/${item.operation}`)} id="{item.id}" options="{leave_only_side_output(item.options)}" {index} add_to_recipe="{undefined}" keybinding={undefined} />
                {:else}
