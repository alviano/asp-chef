<script context="module">
    ...
    const operation = "Fabric.js";
    export const default_extra_options = {
        predicate: Option("__json__", "Predicate containing the Fabric.js configuration (a Base64-encoded JSON)", "predicate_name"),
...
<script>
    ...
    import Component from "./+Fabric.js.svelte";
            ...
            {#each models as model, model_index}
                ...
                {#key model}
                    {#each model.filter(atom => atom.predicate === options.predicate) as configuration}
                        <Component part="{model}" index="{index}" configuration_atom="{configuration}" multistage="{options.multistage}" />
                    {/each}
                {/key}
            {/each}
        </div>
