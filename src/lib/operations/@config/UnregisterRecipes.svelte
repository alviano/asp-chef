...
<script>
    ...
    function get_operations() {
        operations = Recipe.operation_components(filter).filter(operation => Recipe.is_remote_recipe_operation(operation));
    }

    function unregister(operation) {
        Recipe.unregister_remote_recipe_operation(operation);
        get_operations();
    }
        ...
        await Utils.confirm({
            ...
            onconfirm: () => {
                operations.forEach((operation, index) => {
                    Recipe.unregister_remote_recipe_operation(operation, index === operations.length - 1);
